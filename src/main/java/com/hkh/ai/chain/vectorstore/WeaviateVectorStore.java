...
public class WeaviateVectorStore implements VectorStore{
    ...
    public Result<Boolean> createExampleSchema(String kid){
        ...
        WeaviateClass clazz = WeaviateClass.builder()
                .className(className + example + kid)
                .description("local knowledge example")
                .vectorIndexType("hnsw")
                .vectorizer("text2vec-transformers")
                .shardingConfig(shardingConfig)
                .vectorIndexConfig(vectorIndexConfig)
                ...
                .properties(new ArrayList() {{
                    add(Property.builder()
                            .dataType(new ArrayList(){ { add(DataType.TEXT); } })
                            .name("content")
                            .description("The content of the local knowledge example,for search")
                            .moduleConfig(propertyModuleConfigSkipFalse)
                            .build());
                    add(Property.builder()
                            .dataType(new ArrayList(){ { add(DataType.TEXT); } })
                            .name("kid")
                            .description("The knowledge id of the local knowledge example,for search")
                            .moduleConfig(propertyModuleConfigSkipTrue)
                            .build());
                    add(Property.builder()
                            .dataType(new ArrayList(){ { add(DataType.TEXT); } })
                            .name("docId")
                            .description("The doc id of the local knowledge example,for search")
                            .moduleConfig(propertyModuleConfigSkipTrue)
                            .build());
                    add(Property.builder()
                            .dataType(new ArrayList(){ { add(DataType.TEXT); } })
                            .name("uuid")
                            .description("The uuid id of the local knowledge example fragment(same with id properties),for search")
                            .moduleConfig(propertyModuleConfigSkipTrue)
                            .build());
                } })
                .build();
    ...
    @Override
    public void storeExampleEmbeddings(List<String> chunkList, List<List<Double>> vectorList, String kid, String docId, Boolean firstTime) {
        removeExampleByKid(kid);
        if (firstTime) {
            createExampleSchema(kid);
        }
        WeaviateClient client = getClient();
        for (int i = 0; i < chunkList.size(); i++) {
            if (vectorList != null) {
                ...
                dataSchema.put("uuid", uuid);
                Result<WeaviateObject> result = client.data().creator()
                        .withClassName(className + example + kid)
                        .withID(uuid)
                        .withVector(vf)
                        .withProperties(dataSchema)
                        .run();
    ...
    public void removeExampleByKid(String kid) {
        WeaviateClient client = getClient();
        Result<Boolean> result = client.schema().classDeleter().withClassName(className + example + kid).run();
        if (result.hasErrors()) {
            System.out.println(result.getError());
        }else {
            System.out.println(result.getResult());
        }
        log.info("removeExampleByKid result = {}",result);
    }

    @Override
    public List<String> nearestExample(List<Double> queryVector, String kid) {
        if (StringUtils.isBlank(kid)){
            return new ArrayList<String>();
        }
        List<String> resultList = new ArrayList<>();
        ...
        LinkedTreeMap<String,Object> t = (LinkedTreeMap<String, Object>) result.getResult().getData();
        LinkedTreeMap<String,ArrayList<LinkedTreeMap>> l = (LinkedTreeMap<String, ArrayList<LinkedTreeMap>>) t.get("Get");
        ArrayList<LinkedTreeMap> m = l.get(className + example + kid);
        PromptRetriever<ArrayList<LinkedTreeMap>> promptRetriever = promptRetrieverFactory.getPromptRetriever();
        ArrayList<LinkedTreeMap> retrieved = promptRetriever.retrieve(m);
        for (LinkedTreeMap linkedTreeMap : retrieved){
            String content = linkedTreeMap.get("content").toString();
