...
public class MilvusVectorStore implements VectorStore{
    ...
    public void storeExampleEmbeddings(List<String> chunkList, List<List<Double>> vectorList, String kid, String docId, Boolean firstTime) {
        ...
        milvusServiceClient.createPartition(
                CreatePartitionParam.newBuilder()
                        .withCollectionName(collectionName + example + kid)
                        .withPartitionName(docId)
                        .build()
        );
        List<List<Float>> vectorFloatList = new ArrayList<>();
        ...
        milvusServiceClient.insert(insertParam);
        // milvus在将数据装载到内存后才能进行向量计算
        milvusServiceClient.loadCollection(LoadCollectionParam.newBuilder().withCollectionName(collectionName + example + kid).build());
    }
    ...
    private void createExampleSchema(String kid) {
        FieldType primaryField = FieldType.newBuilder()
                .withName("row_id")
                .withDataType(DataType.Int64)
                .withPrimaryKey(true)
        ...
        CreateCollectionParam createCollectionReq = CreateCollectionParam.newBuilder()
                .withCollectionName(collectionName + example + kid)
                .withDescription("local knowledge example")
                .addFieldType(primaryField)
                .addFieldType(contentField)
                .addFieldType(kidField)
                .addFieldType(docIdField)
