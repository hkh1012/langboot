...
public class OpenAiChatService implements ChatService {
    ...
    public void audioChat(CustomChatMessage request, List<String> nearestList, List<Conversation> history, SseEmitter sseEmitter, SysUser sysUser,String mediaId){
        ...
        // 创建一个阻塞队列用于存放语音文本片段
        LinkedBlockingDeque<SseAudioChunk> queue = new LinkedBlockingDeque();
        // 创建一个线程用于监听队列的元素
        new Thread(() -> {
            while (true) {
                try {
                    // 从队列中取出元素
                    SseAudioChunk audioChunk = queue.take();
                    ...
                    if (StrUtil.isBlank(item.getChoices().get(0).getFinishReason())
                        ...
                        if (sb.length() >= splitArr[3] * splitArr[0]){
                            if (content.endsWith("<br>")){
                                ...
                                queue.offer(new SseAudioChunk(splitArr[4],chunk,sseEmitter,false));
                        ...
                        splitArr[4]++;
                        String chunk = sb.substring(splitArr[1], splitArr[2]);
                        queue.offer(new SseAudioChunk(splitArr[4],chunk,sseEmitter,true));
    ...
    private void sseChunkAudio(SseAudioChunk sseAudioChunk,int cid) throws IOException {
