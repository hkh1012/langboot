package com.hkh.sa.base.module.support.securityprotect.service;
...
public class Level3ProtectConfigService {
    ...
    void init() {
        ...
        if (StrUtil.isEmpty(configValue)) {
            throw new ExceptionInInitializerError("sys_config 表 三级等保配置为空，请进行配置！");
        }
        Level3ProtectConfigForm level3ProtectConfigForm = JSONObject.parseObject(configValue, Level3ProtectConfigForm.class);
    ...
    public ResponseDTO<String> updateLevel3Config(Level3ProtectConfigForm configForm) {
        ...
        setProp(configForm);
        // 保存数据库
        String configFormJsonString = JSONObject.toJSONString(configForm);
