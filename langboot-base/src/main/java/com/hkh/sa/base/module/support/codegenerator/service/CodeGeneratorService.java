package com.hkh.sa.base.module.support.codegenerator.service;
...
import com.hkh.domain.constant.CodeGeneratorConstant;
import com.hkh.domain.dto.*;
import com.hkh.domain.entity.CodeGeneratorConfigEntity;
import com.hkh.domain.form.codegenerator.CodeGeneratorConfigForm;
import com.hkh.domain.form.codegenerator.CodeGeneratorPreviewForm;
import com.hkh.domain.form.table.TableQueryForm;
import com.hkh.domain.vo.table.TableColumnVO;
import com.hkh.domain.vo.table.TableConfigVO;
import com.hkh.domain.vo.table.TableVO;
import com.hkh.sa.base.common.util.SmartPageUtil;
import com.hkh.sa.base.common.util.SmartStringUtil;
import com.hkh.sa.base.module.support.codegenerator.dao.CodeGeneratorConfigDao;
import com.hkh.sa.base.module.support.codegenerator.dao.CodeGeneratorDao;
...
public class CodeGeneratorService {
    ...
    public synchronized ResponseDTO<String> updateConfig(CodeGeneratorConfigForm form) {
        ...
        String autoIncreaseKey = null;
        for (TableColumnVO tableColumnVO : tableColumns){
            if ("PRI".equalsIgnoreCase(tableColumnVO.getColumnKey())){
                primaryKey = tableColumnVO.getColumnName();
            }
            if ("auto_increment".equalsIgnoreCase(tableColumnVO.getExtra())){
                autoIncreaseKey = tableColumnVO.getColumnName();
            }
        }
        // 校验表必须有主键
        ...
        for (CodeField codeField : form.getFields()){
            if (primaryKey.equalsIgnoreCase(codeField.getColumnName())){
                codeField.setPrimaryKeyFlag(true);
            }
            if (autoIncreaseKey.equalsIgnoreCase(codeField.getColumnName())){
                codeField.setAutoIncreaseFlag(true);
            }
        }


        codeGeneratorConfigEntity.setTableName(form.getTableName());
        codeGeneratorConfigEntity.setBasic(JSONObject.toJSONString(form.getBasic()));
        codeGeneratorConfigEntity.setFields(JSONArray.toJSONString(form.getFields()));
        codeGeneratorConfigEntity.setInsertAndUpdate(JSONObject.toJSONString(form.getInsertAndUpdate()));
        codeGeneratorConfigEntity.setDeleteInfo(JSONObject.toJSONString(form.getDeleteInfo()));
